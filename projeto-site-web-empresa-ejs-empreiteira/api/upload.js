...
function safeName(name = 'imagem.webp') {
...
export default async function handler(req, res) {
  ...
  if (!type.startsWith('image/')) return res.status(400).json({ error: 'Envie apenas imagens.' });
  const size = Number(req.headers['content-length'] || 0);
  if (size > 12 * 1024 * 1024) return res.status(413).json({ error: 'A imagem deve ter no máximo 12 MB.' });
  const original = safeName(String(req.query.filename || 'imagem.webp'));
