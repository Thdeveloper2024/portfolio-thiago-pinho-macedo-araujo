...
{id:'obra-1',title:'Residência contemporânea fictícia',desc:'Conceito visual demonstrativo com fachada moderna, iluminação arquitetônica e acabamento sofisticado.',cover:'assets/img/hero-stages/07-resultado-final.jpg',gallery:['assets/img/hero-stages/07-resultado-final.jpg']},
{id:'obra-2',title:'Projeto interno contemporâneo',desc:'Ambiente renovado com materiais de visual sofisticado e iluminação valorizada.',cover:'assets/img/hero-ejs.png',gallery:['assets/img/hero-ejs.png']},
{id:'obra-3',title:'Acabamentos e manutenção',desc:'Soluções sob medida para renovar, corrigir e finalizar ambientes.',cover:'assets/img/logo-ejs.png',gallery:['assets/img/logo-ejs.png']}
]};
...
data.works.forEach((w,i)=>{const card=document.createElement('article');card.className='work-card';card.innerHTML=`<img src="${w.cover}" alt="${w.title}"><div class="work-overlay"><div><h3>${w.title}</h3><p>${w.desc||''}</p><a href="obra.html?id=${encodeURIComponent(w.id)}">Ver mais</a></div></div>`;carousel.appendChild(card);const d=document.createElement('button');d.setAttribute('aria-label',`Ir para obra ${i+1}`);if(i===0)d.classList.add('active');d.onclick=()=>card.scrollIntoView({behavior:'smooth',inline:'start',block:'nearest'});dots.appendChild(d)});
